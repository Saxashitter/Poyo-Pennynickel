...
function SaveData_I.WriteSaveFile(fileStr, dataTable, encrypt)
	...
	if (encrypt) then
		file:write(crypt(Serialize(dataTable), enc2))
	else
...
-- poyo time
local SAVE_DATA_PATH = "client/poyo-save.dat"

local function ConstructSaveData()
	local data = {}

	data.VisorColor = 0

	data.EmoteKey = "b"
	data.EmoteConfirmKey = "mouse1"
	data.EmoteLeaveKey = "escape"
	data.EmoteAltKey = "mouse2"

	return data
end

-- reloads the save data
function PoyoPennynickel:LoadFile()
	PoyoPennynickel.SaveData = ConstructSaveData()

	if not SaveData_I.FileExists(SAVE_DATA_PATH) then
		print("not loading a save file")
		return PoyoPennynickel.SaveData
	end

	local data = SaveData_I.ReadSaveFile(SAVE_DATA_PATH, true)
	...
	for k, v in pairs(data) do
		print(k)
		print(v)
		PoyoPennynickel.SaveData[k] = v
	end

	return PoyoPennynickel.SaveData, data
end

-- saves every variable in save data
function PoyoPennynickel:SaveFile()
	SaveData_I.WriteSaveFile(SAVE_DATA_PATH, PoyoPennynickel.SaveData, true)
end

PoyoPennynickel:LoadFile()
