...
addHook("KeyDown", function(keyevent)
	...
	if not PoyoPennynickel.EmoteWheel and keyevent.name == PoyoPennynickel.SaveData.EmoteKey then
	...
	if PoyoPennynickel.EmoteWheel and keyevent.name == PoyoPennynickel.SaveData.EmoteLeaveKey then
		if ALTERNATE_EMOTE then
			ALTERNATE_EMOTE = 0
			ALTERNATE_EMOTE_SELECTION = 0
		else
	...
	and keyevent.name == PoyoPennynickel.SaveData.EmoteAltKey
	...
	if PoyoPennynickel.EmoteWheel and (keyevent.name == PoyoPennynickel.SaveData.EmoteConfirmKey or keyevent.name == PoyoPennynickel.SaveData.EmoteKey) then
