...
local S_PLAY_POYO_BOUNCE = freeslot("S_PLAY_POYO_BOUNCE")

local sfx_py_ada = freeslot("sfx_py_ada")
local sfx_py_bnc = freeslot("sfx_py_bnc")

sfxinfo[sfx_py_ada].caption = "Perfect!!"
sfxinfo[sfx_py_bnc].caption = "Bounce"
...
-- for checking if we hit a perfect
Class.downwardsSwingFrame = 0
...
-- 	if mo.state == S_PLAY_POYO_UPPERCUT then return end
	...
	if class.flags & PAB_BAT == 0 then return end
...
PoyoPennynickel:addScript("PlayerPostUpdate", function(player)
		...
		and P_IsObjectOnGround(mo) then
			if player.cmd.buttons & BT_JUMP then
				...
				if class.downwardsSwingMove then
					class:perfectHUD()
					S_StartSound(mo, sfx_py_ada)
					player.jumpfactor = $*3/2 -- multiply our current bounce
				end
				...
				player.mo.state = S_PLAY_POYO_BOUNCE
				player.jumpfactor = jumpfactor
				player.pflags = $ & ~PF_NOJUMPDAMAGE
				PoyoPennynickel.DustEffect(mo, 8, 10*FU, FU * 35 / 40, 15)
				class:dustTrail(15)
				S_StartSound(mo, sfx_py_bnc)
				return
			end
			mo.momx = $*2
			mo.momy = $*2
			class:doDashAttack()
			PoyoPennynickel.DustEffect(mo, 8, 15*FU, FU * 35 / 42, 15)
	...
	class.downwardsSwingFrame = mo.frame & FF_FRAMEMASK
...
PoyoPennynickel:addScript("PlayerHeight", function(player)
	local mo = player.mo
	local class = mo.poyoChar

	if mo.state == S_PLAY_POYO_BOUNCE then
		return player.spinheight
	end
end)

PoyoPennynickel:addScript("PlayerCanEnterSpinGaps", function(player)
	local mo = player.mo
	local class = mo.poyoChar
